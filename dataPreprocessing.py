...
def normalizeSentenceDataCube(sentenceDat, singleLetterDat):
    ...
    for b in range(sentenceDat["blockList"].shape[0]):
        trialsFromThisBlock = np.squeeze(
            sentenceDat["sentenceBlockNums"] == sentenceDat["blockList"][b]
        )
        trialsFromThisBlock = np.argwhere(trialsFromThisBlock)

        closestIdx = np.argmin(
            np.abs(
                singleLetterDat["blockList"].astype(np.int32)
                - sentenceDat["blockList"][b].astype(np.int32)
            )
        )
...
def prepareDataCubesForRNN(
    ...
    binsPerTrial = np.round(labelsDat["timeBinsPerSentence"] / rnnBinSize).astype(
        np.int32
    )
    ...
    if isTraining:
        ...
        padCombinedTargets = np.zeros(
            [combinedTargets.shape[0], padTo, combinedTargets.shape[2]]
        )
        padErrWeights = np.zeros([errWeights.shape[0], padTo])

        padNeuralData[:, edgeSpace : (edgeSpace + neuralData.shape[1]), :] = neuralData
        padCombinedTargets[:, edgeSpace : (edgeSpace + combinedTargets.shape[1]), :] = (
            combinedTargets
        )
