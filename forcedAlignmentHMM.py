        ...
        for b in range(nBins):
            ...
            if b < (nBins - 1):
                ...
                if x < (len(sentence) - 1):
                    ...
                    A_hmm[currentState, letterStartIdx[x + 1]] = (1 - stayProb) * (
                        1 - blankProb
                    )  # last letter state
                    ...
                    # end of the sentence
                    A_hmm[currentState, currentState + 1] = 1 - stayProb
                    A_hmm[currentState + 1, currentState + 1] = (
                        1.0  # stay in blank permanently (this is the end of the sentence)
                    )
...
def makeTimeWindowMask(stateLabelsSeq, nTimeSteps):
    ...
    for x in range(nStates):
        ...
        tIdx = np.arange(
            np.round(timeFraction * nTimeSteps - winLen * nTimeSteps),
            np.round(timeFraction * nTimeSteps + winLen * nTimeSteps),
        ).astype(np.int32)
...
def hmmForwardBackward(obs, A_hmm, B_hmm, diagVariance, timeWindowMask, startProb):
    ...
    for count in range(1, L):
        ...
        fs[:, count] = (
            ...
            * np.matmul(np.transpose(A_hmm), fs[:, count - 1])
            * timeWindowMask[count - 1, :]
        )
    ...
    for count in range(L - 2, -1, -1):
        ...
        bs[:, count] = np.matmul(
            A_hmm, bs[:, count + 1] * gaussianEmissionProb * timeWindowMask[count, :]
        )
    ...
    for count in range(L):
        ...
        with np.errstate(divide="ignore"):
            v = (
                gaussianEmissionProb
                + np.transpose(maxVal)
                + np.log(timeWindowMask[count, :, np.newaxis])
            )
...
def refineCharacterStartTimes(obs, sentence, templates, letterStarts, letterStretches):
    ...
    for c in range(len(sentence)):
        ...
        possibleStart = np.arange(letterStarts[c] - 50, letterStarts[c] + 55, 5).astype(
            np.int32
        )
        ...
        for stretch, stretchCount in zip(possibleStretch, range(len(possibleStretch))):
            ...
            for colIdx in range(template.shape[1]):
                stretchedTemplate[:, colIdx] = np.interp(
                    newX, np.linspace(0, 1, template.shape[0]), template[:, colIdx]
                )
            ...
            for startIdx, startCount in zip(possibleStart, range(len(possibleStart))):
                ...
                if c > 0:
                    prevTemplateEnd = (
                        letterStarts[c - 1]
                        + letterStretches[c - 1] * templates[sentence[c - 1]].shape[0]
                    )
                ...
                stepIdx = np.arange(
                    startIdx, startIdx + stretchedTemplate.shape[0]
                ).astype(np.int32)
                if stepIdx[-1] >= (obs.shape[0]):
                    continue

                # compute correlation between this template and the data
                msDat = obs[stepIdx, :] - np.mean(
                    obs[stepIdx, :], axis=0, keepdims=True
                )
                msST = stretchedTemplate - np.mean(
                    stretchedTemplate, axis=0, keepdims=True
                )
...
def initializeCharacterTemplates(twCubes, charDef):
    ...
    templates = {}
    for char, charAbbr, thisCharLen in zip(
        charDef["charList"], charDef["charListAbbr"], charDef["charLen"]
    ):
...
def forcedAlignmentLabeling(obs, sentence, templates):
    ...
    hmmBinSize = 5
    blankProb = 0.1
    A_hmm, B_hmm, diagVariance, stateLabels, stateLabelsSeq = makeForcedAlignmentHMM(
        templates, sentence, hmmBinSize, blankProb
    )
    ...
    viterbiStates = hmmViterbi(
        obsBinned, A_hmm, B_hmm, diagVariance, timeWindowMask, startProb
    )
    ...
    for x in range(len(sentence)):
        ...
        letterStretches[x] = (len(thisChar) * hmmBinSize) / templates[
            sentence[x]
        ].shape[0]

    # refine the character start times
    letterStarts, letterStretches = refineCharacterStartTimes(
        obs, sentence, templates, letterStarts, letterStretches
    )
    ...
    blankStart = np.argwhere(
        np.logical_and(paddedStates[1:] == -1, paddedStates[0:-1] != -1)
    )
    blankEnd = (
        np.argwhere(np.logical_and(paddedStates[0:-1] == -1, paddedStates[1:] != -1))
        - 1
    )
    blankWindows = []
    ...
    for b in range(len(blankStart)):
        loopIdx = np.arange(
            (blankStart[b, 0] + 1) * 5 - 1, (blankEnd[b, 0] + 2) * 5 - 2
        ).astype(np.int32)
