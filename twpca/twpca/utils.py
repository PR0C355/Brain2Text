...
def get_uninitialized_vars(sess, var_list=None):
    ...
    is_init = sess.run(list(map(tf.compat.v1.is_variable_initialized, var_list)))
...
def initialize_new_vars(sess, var_list=None):
    ...
    """
    sess.run(tf.compat.v1.variables_initializer(get_uninitialized_vars(sess, var_list)))
