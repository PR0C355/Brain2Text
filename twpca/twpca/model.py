...
class TWPCA(object):
    def __init__(self, data, n_components,
        ...
        self._sess = tf.compat.v1.Session() if sess is None else sess
        ...
        _pos_tau = tf.nn.softplus(self._vars['tau']) / tf.math.log(2.0)
        ...
        self._inv_warp = tf.compat.v1.py_func(warp._invert_warp_indices, _args, tf.float32)
        ...
        self._learning_rate = tf.compat.v1.placeholder(tf.float32, shape=[])
    ...
    def assign_factors(self):
        ...
        if 'time' in self._vars.keys():
            assignment_ops = [tf.compat.v1.assign(self._vars['time'], time_fctr),
                              tf.compat.v1.assign(self._vars['neuron'], neuron_fctr)]
            if self.fit_trial_factors:
                assignment_ops += [tf.compat.v1.assign(self._vars['neuron'], trial_fctr)]
    ...
    def assign_warps(self, warps, normalize_warps=False):
        ...
        if 'warp' in self._params.keys():
            ...
            for _v, v in zip(tau_vars, (shift, scale, tau)):
                ops += [tf.compat.v1.assign(_v, tf.constant(v, dtype=tf.float32))]
            self._sess.run(ops)
        else:
    ...
    def load(self, new_vars):
        ...
        self._sess.run([tf.compat.v1.assign(self._vars[k], v) for k, v in new_vars.items()])
