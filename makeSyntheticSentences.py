...
def generateCharacterSequences(args):
    ...
    synthNeural, synthCharProb, synthCharStart = makeSyntheticDataFromRawSnippets(
    ...
    synthTargets = np.concatenate(
        [synthCharProb, synthCharStart[:, :, np.newaxis]], axis=2
    )

    # cut off the first part of the data so the RNN starts off "hot" randomly in the middle of text
    synthNeural_cut = np.zeros(
        [args["nSentences"], args["nSteps"], synthNeural.shape[2]]
    )
    synthTargets_cut = np.zeros(
        [args["nSentences"], args["nSteps"], synthTargets.shape[2]]
    )

    for t in range(args["nSentences"]):
        randStart = np.random.randint(nStepsToGenerate - args["nSteps"])
        synthNeural_cut[t, :, :] = synthNeural[
            t, randStart : (randStart + args["nSteps"]), :
        ]
        synthTargets_cut[t, :, :] = synthTargets[
            t, randStart : (randStart + args["nSteps"]), :
        ]
    ...
    if args["binSize"] == 1:
        ...
        synthTargetsBinned = np.zeros(
            [args["nSentences"], nBins, synthTargets.shape[2]]
        )
    ...
    for t in range(errWeights.shape[0]):
        charStarts = np.argwhere(
            synthTargetsBinned[t, 1:, -1] - synthTargetsBinned[t, 0:-1, -1] >= 0.1
        ).astype(np.int32)
        if len(charStarts) == 0:
            errWeights[t, :] = 0
        else:
    ...
    with tf.io.TFRecordWriter(args["saveFile"]) as writer:
        for trialIdx in range(args["nSentences"]):
            feature = {
                "inputs": _floats_feature(
                    np.ravel(synthNeuralBinned[trialIdx, :, :]).tolist()
                ),
                "labels": _floats_feature(
                    np.ravel(synthTargetsBinned[trialIdx, :, :]).tolist()
                ),
                "errWeights": _floats_feature(
                    np.ravel(errWeights[trialIdx, :]).tolist()
                ),
            }
...
def makeSyntheticDataFromRawSnippets(
    charDef,
    charSnippets,
    nSentences,
    nSteps,
    ...
    for t in range(nSentences):
        ...
        while currIdx < nSteps:
            ...
            if currentLetterIdx >= len(currentWord):
                currentLetterIdx = 0
                currentWord = pickWordForSentence(
                    wordList,
                    rareLetterIncrease=rareLetterIncrease,
                    rareWordList=rareWordList,
                )
            ...
            if (currentLetterIdx < (len(currentWord) - 1)) and accountForPenState:
                nextClassIdx = charDef["strToCharIdx"][
                    currentWord[currentLetterIdx + 1]
                ]
                ...
                penEndStates = charSnippets[
                    charDef["charList"][classIdx] + "_penEndState"
                ]

                # -2 is a special code that means the snippet can always be used; useful when there isn't enough data
                validIdx = np.argwhere(
                    np.logical_or(
                        penEndStates[0, :] == nextPenStartLoc, penEndStates[0, :] < -1.5
                    )
                )

                if validIdx.shape[0] == 0:
                    # choose a random one
                    choiceIdx = np.random.randint(
                        len(charSnippets[charDef["charList"][classIdx]][0])
                    )
                else:
            ...
            currentSnippet = charSnippets[charDef["charList"][classIdx]][
                0, choiceIdx
            ].copy()
            ...
            synthNeural[t, currIdx : (currIdx + currentSnippet.shape[0]), :] = (
                currentSnippet
            )
...
def extractCharacterSnippets(
    ...
    for sentIdx in range(len(sentences)):
        ...
        for x in range(nChars):
            ...
            if x < (nChars - 1):
                loopIdx = np.arange(
                    letterStarts[sentIdx, x], letterStarts[sentIdx, x + 1]
                ).astype(np.int32)
            else:
                loopIdx = np.arange(
                    letterStarts[sentIdx, x], sentenceLens[sentIdx]
                ).astype(np.int32)
            ...
            if x < (nChars - 1):
                ...
                nextCharIdx = np.squeeze(
                    np.argwhere(np.array(charDef["charListAbbr"]) == nextChar)
                )
            ...
            thisCharIdx = np.squeeze(
                np.argwhere(np.array(charDef["charListAbbr"]) == thisChar)
            )
        ...
        for b in range(len(bw)):
            snippetDict["blank"].append(
                neuralCube[sentIdx, bw[0, b][bw[0, b] < neuralCube.shape[1]], :]
            )
...
def addSingleLetterSnippets(snippetDict, slDat, twCubes, charDef):
    ...
    for charIdx in range(len(charDef["charList"])):
        ...
        for b in range(slDat["blockList"].shape[0]):
            ...
            neuralCube[trialsFromThisBlock, :, :] -= slDat["meansPerBlock"][
                np.newaxis, b, :
            ]
        ...
        for t in range(neuralCube.shape[0]):
            endStep = np.argwhere(
                twCubes[char + "_T"][:, t] >= 60 + charDef["charLen"][charIdx]
            ).astype(np.int32)
            if len(endStep) == 0:
                continue
