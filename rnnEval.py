...
def evaluateRNNOutput(
    ...
    decStr = decodeCharStr(
        lgit,
        charStart,
        charStartThresh,
        charStartDelay,
        numBinsPerSentence,
        charDef["charListAbbr"],
    )
...
def decodeCharStr(
    logitMatrix, transSignal, transThresh, transDelay, numBinsPerTrial, charList
):
    """
    ...
    for v in range(logitMatrix.shape[0]):
        ...
        transIdx = np.argwhere(
            np.logical_and(letTrans[0:-1] < transThresh, letTrans[1:] > transThresh)
        )
        transIdx = transIdx[:, 0]
...
def rnnOutputToKaldiMatrices(rnnOutput, numBinsPerSentence, charDef, kaldiDir):
    ...
    for t in range(rnnOutput.shape[0]):
        ...
        charProb = np.transpose(
            probCombined[t, startIdx:endIdx:5, charDef["idxToKaldi"]]
        )

        charProb[charProb == 0] = 1e-13
        charProb = np.log(charProb)

        writeKaldiProbabilityMatrix(
            charProb, t, kaldiDir + "kaldiMat_" + str(t) + ".txt"
        )
