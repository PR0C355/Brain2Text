...
def labelDataset(
    sentenceDat, singleLetterDat, twCubes, trainPartitionIdx, testPartitionIdx, charDef
):
    """
    ...
    neuralCubeSmoothed = scipy.ndimage.filters.gaussian_filter1d(
        normalizedNeuralCube, 4.0, axis=1
    )
    ...
    if len(testPartitionIdx) > 0:
        trainAndTestIdx = np.squeeze(
            np.concatenate([trainPartitionIdx, testPartitionIdx], axis=1)
        )
    else:
    ...
    for iterIdx in range(nHMMIters):
        ...
        for x in trainAndTestIdx:
            args.append(
                [
                    neuralCubeSmoothed[
                        x, 0 : sentenceDat["numTimeBinsPerSentence"][x, 0], :
                    ],
        ...
        for x in np.squeeze(trainPartitionIdx):
            ...
            for c in range(nChars):
                ...
                timeSteps = np.arange(
                    letterStarts[x, c], letterStarts[x, c] + letterDurations[x, c]
                ).astype(np.int32)
                ...
                for e in range(resampleDat.shape[1]):
                    resampleDat[:, e] = np.interp(
                        np.linspace(0, 1, resampleDat.shape[0]),
        ...
        for x in charList:
            if (
                charCount[x] > 0
            ):  # avoid dividing by zero and causing a warning message for rare letters that may have no examples
...
def constructRNNTargets(
    letterStarts, letterDurations, maxTimeSteps, sentences, charDef
):
    """
    ...
    for sentenceIdx in range(nSentences):
        ...
        for x in range(len(sentence)):
            ...
            if x < (len(sentence) - 1):
                ...
                stepIdx = np.arange(
                    letterStarts[sentenceIdx, x], letterStarts[sentenceIdx, x + 1]
                ).astype(np.int32)
            else:
                # last character - extend until end of sentence
                stepIdx = np.arange(letterStarts[sentenceIdx, x], maxTimeSteps).astype(
                    np.int32
                )

            # now fill in these time steps with a '1' for the given character
            charIdx = np.squeeze(
                np.argwhere(np.array(charDef["charListAbbr"]) == sentence[x])
            )
            ...
            stepIdx = np.arange(
                letterStarts[sentenceIdx, x], letterStarts[sentenceIdx, x] + 21
            ).astype(np.int32)
        ...
        charIdx = np.squeeze(
            np.argwhere(np.array(charDef["charListAbbr"]) == sentence[0])
        )
        charProbTarget[
            sentenceIdx, 0 : letterStarts[sentenceIdx, 0].astype(np.int32), charIdx
        ] = 1
        ignoreErrorHere[
            sentenceIdx, 0 : letterStarts[sentenceIdx, 0].astype(np.int32)
        ] = 1
